...
import { RefreshCw, Users, CalendarCheck, Trophy, TrendingUp, Settings, X, CheckCircle } from "lucide-react";
...
const STORAGE_KEY_CANDIDATES = "ta_csv_candidates";
const STORAGE_KEY_INTERVIEWS = "ta_csv_interviews";
...
export default function TADashboard() {
  ...
  const [savedUrls, setSavedUrls] = useState({ candidates: "", interviews: "" });
  const [saveSuccess, setSaveSuccess] = useState(false);

  // Load saved URLs from localStorage on mount
  useEffect(() => {
    const c = localStorage.getItem(STORAGE_KEY_CANDIDATES) || "";
    const i = localStorage.getItem(STORAGE_KEY_INTERVIEWS) || "";
    setCandidateUrl(c);
    setInterviewUrl(i);
    setSavedUrls({ candidates: c, interviews: i });
    if (!c || !i) setShowSettings(true);
  }, []);

  const saveUrls = () => {
    localStorage.setItem(STORAGE_KEY_CANDIDATES, candidateUrl.trim());
    localStorage.setItem(STORAGE_KEY_INTERVIEWS, interviewUrl.trim());
    setSavedUrls({ candidates: candidateUrl.trim(), interviews: interviewUrl.trim() });
    setSaveSuccess(true);
    ...
    try {
      ...
      const rows: FunnelRow[] = allRoles
        .map(role => {
          ...
          return { role: role.replace(/\b\w/g, l => l.toUpperCase()), screened: s, interviewed: iv, selected: sel, conversionRate: s > 0 ? Math.round((sel / s) * 100) : 0 };
        })
      ...
      setError("Failed to load data. Make sure your sheet is published as CSV.");
      console.error(e);
    }
    setLoading(false);
      ...
      {showSettings && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)" }}>
          <div className="w-full max-w-lg mx-4 rounded-2xl border border-white/10 p-6" style={{ background: "#0f0f1a" }}>
            ...
            <div className="mb-2 p-3 rounded-lg text-xs text-white/50 border border-white/5" style={{ background: "rgba(129,140,248,0.05)" }}>
              In Google Sheets: <strong className="text-white/70">File → Share → Publish to web</strong> → select each tab → choose <strong className="text-white/70">CSV</strong> → Publish → copy the link
            </div>

            <div className="space-y-4 mt-4">
              <div>
                <label className="block text-xs text-white/40 uppercase tracking-wider mb-2">Candidate Tracker CSV URL</label>
                <input
                  value={candidateUrl}
              ...
              <div>
                <label className="block text-xs text-white/40 uppercase tracking-wider mb-2">Interview Tracker CSV URL</label>
                <input
                  value={interviewUrl}
            ...
            <button
              onClick={saveUrls}
              disabled={!candidateUrl.trim() || !interviewUrl.trim()}
